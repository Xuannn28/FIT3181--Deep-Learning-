...
* **Key Components:**
  ...
  * **Entropy-Regularized Loss Formulation:** Implementation of a composite loss function tracking cross-entropy penalty and prediction distribution uncertainty ($\mathcal{H}(p)$) simultaneously, using adjustable scaling constants ($\lambda$) to penalize overconfident predictions.
  * **Sharpness-Aware Minimization (SAM) Implementation:** Custom low-level parameter optimization mechanics mapping two-step gradient trajectories. The approach computes geometric neighborhood norm steps to dynamically scale gradient updates, shifting weights into flatter minima regions to structurally increase generalization capacity.
* **Techniques Used:**
